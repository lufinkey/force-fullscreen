/*
This file has been modified from another software project
Check out https://github.com/jossef/fullscreen-window for the original
*/

const TabCinema = {};

TabCinema.styleContent = `
.tc-overlay {
...
TabCinema.initialize = function()
{
	...
	this.iframes = {};
	this.target = {};
	this.scrollBeforeMaximize = {top: 0, left: 0};
...
TabCinema.findVideos = function(path=[], callback=null)
{
	...
	if (this.pendingReports === 0) {
		...
		this.wrapUpTimeout = window.setTimeout(() => {
			...
			if (window.location.host === "www.youtube.com") {
				...
				if (controls) {
					...
					if (typeof controls['originalStyle'] === "undefined") {
						...
						this.addClassDeep(controls, 'tc-show');
					}
...
TabCinema.minimizeVideo = function()
{
	...
	if (this.target.tag === "iframe") {
		...
		if (this.target.isYTHTML5) {
			...
			this.removeClassDeep(controls, 'tc-show');
		}
		else {
			this.target.customControls.destroy();
...
TabCinema.handleMutations = function(mutations)
{
	if (!this.hasClass(document.body, "tc-show")) {
		...
		this.handleVideoRemoveTimeout = window.setTimeout(() => {
			this.handleVideoRemoved()
		}, 100);
	}
	else {
		mutations.forEach((mutation) => {
			for (var i in mutation.removedNodes) {
				if (this.hasClass(mutation.removedNodes[i], 'tc-show')) {
					window.clearTimeout(this.handleVideoRemoveTimeout);
					this.handleVideoRemoveTimeout = window.setTimeout(() => {
						this.handleVideoRemoved()
					}, 100);
...
TabCinema.addClassDeep = function(el, cls)
{
	...
	for (var i in children) {
		this.addClassDeep(children[i], cls);
	}
}
...
TabCinema.removeClassDeep = function(el, cls)
{
	...
	var children = el.childNodes;
	for (var i in children) {
		this.removeClassDeep(children[i], cls);
	}
}
